      ...
      <div class="row">
        <div class="col-md-2">
          ...
          <a href="<?php echo empty($item['url']) ? $this->url("product/{$item['product_id']}") : $this->e($item['url']); ?>">
            <img class="img-fluid" title="<?php echo $this->e($item['title']); ?>" alt="<?php echo $this->e($item['title']); ?>" src="<?php echo $this->e($item['thumb']); ?>">
          </a>
          <?php } ?>
        </div>
        <div class="col-md-7">
          ...
          <?php if(!empty($buttons)) { ?>
            <div class="text-right">
              <form method="post" class="product-action">
                ...
                <button title="<?php echo $this->text('Remove'); ?>" class="btn" data-ajax="true" name="remove_from_wishlist" value="1">
                  <i class="fa fa-trash"></i>
                </button>
                <?php } ?>
                <?php if (in_array('compare_remove', $buttons)) { ?>
                <button title="<?php echo $this->text('Remove'); ?>" class="btn" name="remove_from_compare" value="1">
                  <i class="fa fa-trash"></i>
                </button>
                <?php } ?>
