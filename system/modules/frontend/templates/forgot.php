<div class="row">
  <form method="post" id="forgot" class="forgot col-md-6<?php echo isset($form_errors) ? ' form-errors' : ''; ?>">
    ...
    ?>">
      <label><?php echo $this->text('Password');
    ?></label>
      <input class="form-control" type="password" pattern=".{<?php echo $min_password_length;
    ?>,<?php echo $max_password_length;
    ?>}" maxlength="<?php echo $max_password_length;
    ?>" name="user[password]" placeholder="<?php echo $this->text('Minimum @num characters', array('@num' => $min_password_length));
    ?>" autofocus required>
    ...
    ?>">
      <label><?php echo $this->text('E-mail');
    ?></label>
      <input type="email" class="form-control" maxlength="255" name="user[email]" value="<?php echo isset($user['email']) ? $user['email'] : '';
    ?>" autofocus required>
