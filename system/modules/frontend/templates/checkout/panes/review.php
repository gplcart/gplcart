  ...
  <div class="panel-body">
      ...
      <div class="form-group<?php echo isset($this->errors['cart']['items'][$sku]) ? ' has-error' : ''; ?>">
        ...
        <div class="col-md-6">
          ...
          <div>
            <div class="input-group input-group-sm">
              <input class="form-control text-center" maxlength="2"
                     name="order[cart][items][<?php echo $sku; ?>][quantity]" value="<?php echo $item['quantity']; ?>">
            </div>
            <div class="input-group input-group-sm hidden-js">
              <button title="<?php echo $this->text('Update'); ?>" class="btn btn-default btn-sm" name="update"
                      value="1">
        ...
        <div class="col-md-3">
          <div class="btn-toolbar">
            <button title="<?php echo $this->text('Delete'); ?>" class="btn btn-default btn-sm"
                    name="order[cart][action][delete]" value="<?php echo $item['cart_id']; ?>">
              <i class="fa fa-trash"></i>
            </button>
            <button title="<?php echo $this->text('Move to wishlist'); ?>" class="btn btn-default btn-sm"
                    name="order[cart][action][wishlist]" value="<?php echo $sku; ?>">
          ...
          <?php foreach ($price_components as $id => $price_component) { ?>
            <tr>
              <td>
                ...
                <input type="hidden" name="order[data][components][<?php echo $id; ?>]"
                       value="<?php echo $price_component['price']; ?>">
              </td>
              <td>
                <?php if (isset($price_component['rule']['code']) && $price_component['rule']['code'] !== '') { ?>
                  <div
                      class="form-group col-md-8<?php echo isset($this->errors['pricerule_code']) ? ' has-error' : ''; ?>">
                    <div class="input-group">
                      <input class="form-control col-md-2" name="order[data][pricerule_code]"
                             placeholder="<?php echo $this->text('Enter your discount code'); ?>"
                             value="<?php echo isset($order['data']['pricerule_code']) ? $order['data']['pricerule_code'] : ''; ?>">
                      <span class="input-group-btn">
                    <button class="btn btn-default" name="check_pricerule"
                            value="<?php echo $id; ?>"><?php echo $this->text('Check'); ?></button>
                  </span>
                    </div>
                  </div>
                <?php } else { ?>
    ...
    <div class="form-group">
      <div class="col-md-6 hidden-js">
        <button class="btn btn-default" name="update" value="1"><i
              class="fa fa-refresh"></i> <?php echo $this->text('Update'); ?></button>
      </div>
      <div class="col-md-6">
