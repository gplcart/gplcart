...
<div class="row">
  <div class="col-md-12">
    <table class="table table-responsive margin-top-20 categories">
        <thead>
          <tr>
            ...
            <th><?php echo $this->text('Title');
    ?></th>
            <th><?php echo $this->text('Alias');
    ?></th>
            <th><?php echo $this->text('Enabled');
    ?></th>
            <th><?php echo $this->text('Weight');
    ?></th>
            <th></th>
          </tr>
        </thead>
    ...
    ?>">
          <td class="middle"><input type="checkbox" class="select-all" name="selected[]" value="<?php echo $category['category_id'];
    ?>"></td>
          <td class="middle"><?php echo $category['indentation'];
    ?><?php echo $this->escape($category['title']);
    ?></td>
          <td class="middle">
            <a target="_blank" href="<?php echo $this->escape($category['url']);
    ?>"><?php echo $this->truncate($this->escape($category['alias']));
    ?></a>
          </td>
        <td class="middle">
    ...
    ?>
        ...
        <td class="middle">
            <i class="fa fa-arrows handle"></i> <span class="weight"><?php echo $this->escape($category['weight']);
    ?></span>
        </td>
          <td class="text-right">
            <div class="btn-group">
